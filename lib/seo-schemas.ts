 ...
 * - 페이지 특화(BreadcrumbList·Course·FAQPage·Person 상세)는 각 페이지에서
 *   PageJsonLd 로 호출.
 *
...
/** Person — 권동원 사이트 전체 식별자 (다른 페이지/Course에서 @id 참조) */
...
export function buildPersonDongwonDetailed() {
  return {
    ...
    jobTitle: ['액팅 코치', '연기 강사', '배우', 'KD4 대표'],
    description:
      'KD4 액팅 스튜디오 대표. 마이즈너 테크닉 액팅 코치이자 현역 배우로 Disney+ 무빙2, Netflix 중증외상센터 등에 출연 중. 프로 배우 400명 이상을 지도한 경력 보유.',
    url: `${SITE_URL}/acting-coach-dongwon-kwon`,
    image: `${SITE_URL}${DIRECTOR.photo}`,
    worksFor: { '@id': `${SITE_URL}#org` },
    knowsAbout: [
