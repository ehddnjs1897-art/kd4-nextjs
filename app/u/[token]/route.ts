 ...
 *
 * token = actor.id UUID의 첫 8자리(첫 하이픈 앞 그룹).
 * 문자에 넣던 90자짜리 /api/casting-alerts/unsubscribe?id={uuid} 를
 * https://kd4.club/u/3fafa3ce (28자)로 줄이기 위한 리다이렉트 전용 라우트.
 *
 * 동작: 앞 8자리로 배우를 유일하게 특정되면 기존 확인 페이지로 302.
 * 실제 수신거부는 그 페이지의 확정 버튼(POST)으로만 — 여기선 DB 쓰기 없음.
 * 보안: UUID는 공개 프로필 URL과 동일 값이라 앞 8자 노출이 추가 위험을 만들지 않음
 * (기존과 동일하게 POST 확인 단계가 오탐·제3자 해지를 막는다).
 */
