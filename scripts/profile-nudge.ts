...
function buildText(name: string, missing: Missing): string {
  ...
  // 2026-07-22 대표 지시: "왜 업데이트해야하는지 이유도 달아" — 배우 입장에서 목적이
  // 안 와닿는 항목(가로사진)은 결과(공유 시 어떻게 보이는지)까지 구체적으로 적어야
  // 실제 행동으로 이어짐(안세영님 "그냥 프로필 사진 란에 추가하면 되나요?" 문의로 확인).
  if (missing.noLandscape) items.push('▪ 가로(와이드) 사진 최소 1장 — 이게 없으면 카카오톡으로 프로필 공유할 때 세로사진이 억지로 잘려서 어색하게 나와요. 가로로 찍은 사진 한 장만 "＋ 사진 추가"에 올려주시면 캐스팅 담당자에게 공유될 때 훨씬 깔끔하게 보입니다')
