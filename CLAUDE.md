...
### 🚨 Supabase 무료 플랜 전환 (2026-07-21 대표 통보 — 사용량 초과 각별 유의)
- 유료 구독 해지됨. 무료 한도: **DB 500MB · 스토리지 1GB · egress 월 5GB** · 자동 백업 없음
- 실측(7/21): 스토리지 224MB/1,024MB (casting 68 + monologue-cards 130 + actor-photos 22 + docs 4)
- 작업 원칙: ① 스토리지 대량 업로드 금지 — 큰 파일(영상 등)은 R2로 ② 스크립트에서 스토리지 이미지 대량 다운로드 금지 — 가로사진 판정은 `photo-dims-cache.json` 캐시 재사용 ③ 전수 SELECT 반복 자제 — 필요한 컬럼만 ④ 자동 백업 없으므로 스키마 변경·대량 UPDATE 전 수동 백업
